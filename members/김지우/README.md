...
## 0519 제출 파일
- `notebooks/김지우_0519_전처리.ipynb` — 풀 파이프라인 (수집→토큰→eval→검정, 129 cells, 77기업×3=210 firm-year)
- `reports/김지우_전처리.docx` — 전체 표본 검증 보고서
- 핵심: 3종 전처리 실험(exp_B/E/F) robustness + Bootstrap CI + Mann-Whitney + OLS/Ordered Logit. **파일럿 n=30 ρ=+0.444 → 전체 n=210 ρ=-0.197 부호 역전** 발견(상위 대형주 표집 편의 진단).
