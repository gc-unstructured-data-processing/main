// ESG DART v0.6 제출본 빌드 스크립트
// 중심: Talk–Walk Gap(말–실행 격차) 알파 + 축 간 선택적 공시
// 형식: 가이드 준수 — 맑은 고딕 10.5pt, 줄간격 160%, A4 여백 25mm, 양쪽정렬, 페이지번호
const {
...
// 인라인 강조가 필요한 본문 (parts: [텍스트, bold?] 배열)
function rich(parts, opts = {}) {
  return new Paragraph({
    alignment: AlignmentType.BOTH,
    spacing: { before: 0, after: SP_AFTER, line: LS_BODY, lineRule: 'auto' },
    ...opts,
    children: parts.map(p => new TextRun({ text: p[0], font: FONT, size: SZ_BODY, bold: !!p[1] }))
  });
}
...
// ===================== TITLE PAGE =====================
C.push(center('말과 실행은 얼마나 어긋나는가', SZ_TITLE, { bold: true, spacing: { before: 520, after: 0, line: 360 } }));
C.push(center('사업보고서 ESG 공시 언어와 KCGS 등급의 정합성 진단:', SZ_TITLE, { bold: true, spacing: { before: 60, after: 60, line: 360 } }));
...
C.push(body(
  'ESG 등급 자체의 신뢰성은 국내외에서 거듭 지적되어 왔다. 유지선(2025)은 KCGS와 글로벌 평가사 등급의 격차가 정보환경에 따라 ' +
...
C.push(rich([
  ...
  ['하나의 점수', true],
  ['로 합쳐 보기 때문에, 한 기업이 어떤 축은 부풀리고 다른 축에는 침묵하는 축 간(cross-pillar) 선택을 구조적으로 잡아내지 못한다. 기후 공시의 선택적 공개를 다룬 Bingler 외(2022)도 환경(E) 내부의 선택일 뿐 E·S·G 세 축을 각 축의 등급에 견준 것은 아니다.', false]
]));
C.push(rich([
...
C.push(rich([
  ...
  ['격차 = 말 − 실행', true],
  ['이다. 네 가지 결정으로 격차를 구성한다. (1) 분량 착시 제거 — 어휘 강도를 log_n_tokens에 회귀한 잔차만 사용해 길이로 설명되는 몫을 ' +
   '걷어낸다. (2) 단위 통일 — 말과 등급을 각각 z-점수로 표준화한다. (3) 격차 정의 — gap = z(말) − z(등급)으로, 양수는 과잉(말이 앞섬), ' +
   '음수는 과소(등급이 앞섬)다. (4) 차원 — E·S·G 각각과 종합 gap을 따로 본다. 이 격차 구성은 greenwashing 연구가 쓰는 ' +
   '"공시 − 성과" 발상과 같되, 본 표본의 최대 교란인 분량을 talk 항에서 직접 제거한다는 점이 다르다.', false]
]));
...
C.push(body(
  '남은 G 신호가 어디서 오는지 보면, 이사회 등 기관을 다루는 VI 섹션의 거버넌스 어휘가 전체 문서보다 등급과 더 선명하게 연관되고' +
  '(전체 0.426 대 VI 0.526), 이 패턴은 사전에 의존하지 않는 의미 임베딩 분류에서도 재현된다(VI 문장의 97.9%가 G로 분류). ' +
  '거버넌스 신호는 의무공시 보편어가 아니라 실천 어휘에서, 그리고 그것이 공시되는 섹션에서 더 또렷해진다.'
));
C.push(fig('fig5_localization_heatmap.png', 360, 270, '섹션 국소화 히트맵'));
C.push(cap('그림 5. 섹션별 어휘 강도와 차원 등급 간 Spearman ρ. VI_G(0.526)가 전체 문서 G 상관(0.426)을 넘어 가장 진하다.'));
...
C.push(rich([
  ...
  ['전체의 44%(168/381)가 말과 실행이 어긋난다.', true],
...
C.push(fig('fig6_talkwalk_map.png', 500, 229, 'Talk-Walk Gap 2x2 유형 지도'));
C.push(cap('그림 6. 말–실행 격차 2×2 유형 지도. 가로축=분량을 걷어낸 어휘강도(말), 세로축=KCGS 등급(실행). 우하 빨강=과잉공시, 좌상 청록=과소공시.'));
C.push(rich([
  ...
  ['에 가깝다. 그리고 보고서 길이와 규모를 동시에 제거해도 격차는 거의 그대로였다(순위상관 ρ=0.993, 유형 일치율 90.9%). ' +
   '격차는 단순한 규모 대리가 아니라 규모로 환원되지 않는 독립적 정합성 신호다.', false]
]));
...
C.push(fig('fig7_crosspillar.png', 500, 190, '축 간 선택적 공시'));
C.push(cap('그림 7. 축 간 선택적 공시. (왼쪽) 선택적 47% 대 균형 53%. (오른쪽) 각 축이 ‘가장 부풀린 축’·‘가장 침묵한 축’으로 뽑힌 빈도 — G가 양쪽 모두 1위.'));
...
C.push(rich([
  ...
  ['공시의 ESG 어휘량을 액면 그대로 믿으면 분량·규모 착시에 노출된다. 합산 점수만 보면 축 편향을 놓치므로, 비슷한 규모끼리 비교하고 ' +
   '축별(E/S/G)로 말–실행 정합성을 따로 점검해야 한다. 거버넌스 어휘, 특히 VI 섹션의 실천 서술은 외부 평가를 보완하는 독립 정보가 될 수 있다.', false]
]));
C.push(rich([
  ['평가기관: ', true],
...
Packer.toBuffer(doc).then(buf => {
  const out = path.join(__dirname, '보고서_v6_제출본.docx');
